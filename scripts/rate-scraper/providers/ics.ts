		...
		if (
			...
			$(table)
				...
				.each((_, row) => {
					...
					const fixed3AprMatch = fixed3Cell.match(/APRC[\s\-:]*(\d+\.?\d*)/i);
					...
					const fixed5AprMatch = fixed5Cell.match(/APRC[\s\-:]*(\d+\.?\d*)/i);
...
function getMinLtv(currentLtv: number, rows: { ltv: number }[]): number {
	const sortedLtvs = [...new Set(rows.map((r) => r.ltv))].sort((a, b) => a - b);
...
async function parseBtlRates(): Promise<MortgageRate[]> {
	...
	let currentInvestorType: "individual" | "company" | "pension" = "individual";

	// Process tables for different investor types
	$("table").each((_, table) => {
		// Check the heading above this table
		const prevHeading = $(table).prevAll("h2, h3, h4, h5, h6").first().text();
...
function validateStructure(
	...
	if (!hasValidTable) {
		return {
			valid: false,
			error:
				"No valid rate table found on main page (expected Variable and Year columns)",
		};
	}
