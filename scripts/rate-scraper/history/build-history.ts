		...
		if (existing) {
			...
			if (current && previousHash === current.hash) {
				...
				if (connectionIndex >= 0) {
					...
					const changesetsToAppend = existing.changesets.slice(connectionIndex);
					if (changesetsToAppend.length > 0) {
						history.changesets.push(...changesetsToAppend);
						previousHash =
							changesetsToAppend[changesetsToAppend.length - 1].afterHash;
					}
				} else {
					...
					if (existing.changesets.length > 0) {
						...
						previousHash =
							existing.changesets[existing.changesets.length - 1].afterHash;
					}
				}
			}
		}
...
export function previewHistory(history: RatesHistoryFile): void {
	...
	if (history.changesets.length > 0) {
		...
		for (const changeset of history.changesets) {
			const adds = changeset.operations.filter((o) => o.op === "add").length;
			...
			console.log(`    After hash: ${changeset.afterHash.slice(0, 12)}...`);
		}
	} else {
