...
test.describe("Cashback Comparison Calculator", () => {
	...
	test.describe("page load", () => {
		...
		test("displays form card with title", async ({ page }) => {
			await expect(page.getByText("Compare Cashback Options")).toBeVisible();
		});

		test("displays compare button disabled initially", async ({ page }) => {
			const button = page.getByRole("button", { name: /compare options/i });
	...
	test.describe("option management", () => {
		...
		test("can remove an option when more than 2 exist", async ({ page }) => {
	...
	test.describe("rate input modes", () => {
		test("can switch to manual rate entry for an option", async ({ page }) => {
			// Click "Enter manually" tab for first option
			await page.getByRole("tab", { name: "Enter manually" }).first().click();
	...
	test.describe("cashback configuration", () => {
		...
		test("can enter cashback percentage", async ({ page }) => {
			const cashbackInput = page.locator("input[placeholder='2']").first();
	...
	test.describe("form validation and calculation", () => {
		test("enables compare button when form is complete", async ({ page }) => {
			await fillCurrencyInput(page, "Mortgage Amount", 300000);

			// Enter rates for both options manually
			const manualTabs = page.getByRole("tab", { name: "Enter manually" });
			await manualTabs.first().click();
			await page.locator("input[placeholder='e.g. 3.45']").first().fill("3.5");

			await manualTabs.nth(1).click();
			await page.locator("input[placeholder='e.g. 3.45']").nth(1).fill("3.8");

			const button = page.getByRole("button", { name: /compare options/i });
			await expect(button).not.toBeDisabled();
		});

		test("opens result dialog when compare is clicked", async ({ page }) => {
			await fillCurrencyInput(page, "Mortgage Amount", 300000);
			...
			await page.getByRole("button", { name: /compare options/i }).click();
