...
test.describe("Remortgage Breakeven Calculator", () => {
	...
	test.describe("page load", () => {
		...
		test("displays form card with title", async ({ page }) => {
			await expect(page.getByText("Is Switching Worth It?")).toBeVisible();
		});
	...
	test.describe("form inputs", () => {
		...
		test("displays current rate input", async ({ page }) => {
			await expect(page.getByLabel(/Current Interest Rate/)).toBeVisible();
		});
	...
	test.describe("advanced options", () => {
		...
		test("expands advanced options on click", async ({ page }) => {
			...
			await expect(page.getByLabel(/Cashback from New Lender/)).toBeVisible();
			await expect(page.getByLabel(/Early Repayment Charge/)).toBeVisible();
		});
	});
