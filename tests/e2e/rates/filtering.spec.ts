...
test.describe("Rates Page - Filtering", () => {
	...
	test("filter by lender shows only that lender's rates", async ({ page }) => {
		...
		await page.getByRole("menuitemcheckbox", { name: /allied irish/i }).click();
	...
	test("filter by rate type shows only fixed or variable", async ({ page }) => {
