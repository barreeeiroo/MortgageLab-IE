...
function createSelfBuildConfig(
	...
	const defaultStages = [
		...
		{ id: "d3", month: 8, amount: mortgageAmount * 0.4, label: "Completion" },
	];
...
describe("Milestone Calculation Integration Tests", () => {
	describe("standard mortgage milestones", () => {
		...
		it("calculates 25% paid off milestone", () => {
			...
			expect(milestone25?.value).toBeLessThanOrEqual(mortgageAmount * 0.75);
		});
		...
		it("calculates 75% paid off milestone", () => {
			...
			expect(milestone75?.value).toBeLessThanOrEqual(mortgageAmount * 0.25);
		});
	...
	describe("LTV 80% milestone", () => {
		it("includes LTV 80% milestone when starting above 80% LTV", () => {
			...
			expect(ltv80Milestone?.value).toBeLessThanOrEqual(propertyValue * 0.8);
		});
	...
	describe("edge cases", () => {
		it("returns empty array for empty months", () => {
			const milestones = calculateMilestones([], 30000000, 35000000, undefined);
