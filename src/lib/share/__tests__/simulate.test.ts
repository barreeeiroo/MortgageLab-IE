...
describe("simulate share", () => {
	describe("basic simulation state", () => {
		...
		it("roundtrips basic state", () => {
			...
			expect(parsed?.state.input.startDate).toBe(basicState.input.startDate);
		...
		it("preserves rate period properties except id", () => {
			...
			expect(parsedPeriod?.durationMonths).toBe(originalPeriod.durationMonths);
	...
	describe("overpayment configs", () => {
		...
		it("roundtrips recurring overpayment with quarterly frequency", () => {
			...
			expect(parsed?.state.overpaymentConfigs[0]?.frequency).toBe("quarterly");
		});
	...
	describe("custom rates", () => {
		...
		it("embeds used custom rates in URL", () => {
			const url = generateSimulateShareUrl(stateWithCustomRate, [customRate]);
			const params = new URL(url).searchParams;
			const encoded = params.get(SIMULATE_SHARE_PARAM);
			...
			expect(parsed?.embeddedCustomRates[0].name).toBe("My Custom Fixed Rate");
