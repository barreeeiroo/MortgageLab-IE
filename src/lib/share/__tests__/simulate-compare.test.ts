...
describe("simulate-compare share", () => {
	...
	describe("roundtrip encoding/decoding", () => {
		it("preserves simulation input values", () => {
			...
			expect(parsed?.simulations[0].state.input.mortgageAmount).toBe(35000000);
			expect(parsed?.simulations[0].state.input.mortgageTermMonths).toBe(360);
			expect(parsed?.simulations[0].state.input.propertyValue).toBe(45000000);
			expect(parsed?.simulations[0].state.input.startDate).toBe("2026-01-15");
		...
		it("preserves rate periods", () => {
			...
			expect(parsed?.simulations[0].state.ratePeriods[0].lenderId).toBe("boi");
			expect(parsed?.simulations[0].state.ratePeriods[0].durationMonths).toBe(
				60,
			);
			expect(parsed?.simulations[0].state.ratePeriods[1].durationMonths).toBe(
				0,
			);
		});
		...
		it("preserves overpayment configs", () => {
			...
			expect(parsed?.simulations[0].state.overpaymentConfigs).toHaveLength(1);
			expect(parsed?.simulations[0].state.overpaymentConfigs[0].amount).toBe(
				500000,
			);
			expect(parsed?.simulations[0].state.overpaymentConfigs[0].type).toBe(
				"one_time",
			);
		});
		...
		it("embeds custom rates used by simulations", () => {
			...
			expect(parsed?.simulations[0].customRates[0].name).toBe("My Custom Rate");
