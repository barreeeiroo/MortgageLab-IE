    ...
    (simulations): CompareChartDataPoint[] => {
        ...
        for (let yearIndex = 0; yearIndex < maxYears; yearIndex++) {
            ...
            const yearData =
                ...
                simulations.find((s) => s.yearlySchedule[yearIndex])
                    ?.yearlySchedule[yearIndex];
            ...
            for (const sim of simulations) {
                ...
                if (yearData) {
                    ...
                    point[`${prefix}_principalPortion`] =
                    ...
                    if (propertyValue > 0) {
                        ...
                        point[`${prefix}_equity`] =
                            propertyValue - yearData.closingBalance;
                    }
                    ...
                    const baselineMonth =
                        sim.baselineSchedule[yearEndMonth - 1];
                    if (baselineMonth) {
                        point[`${prefix}_baseline`] =
...
export const $compareMonthlyChartData = computed(
    ...
    (simulations): CompareChartDataPoint[] => {
        ...
        const longestSim = simulations.reduce(
            (longest, sim) =>
                sim.amortizationSchedule.length >
                longest.amortizationSchedule.length
                    ? sim
                    : longest,
            simulations[0],
        );
        ...
        for (let monthIndex = 0; monthIndex < maxMonths; monthIndex++) {
            ...
            for (const sim of simulations) {
                ...
                if (monthData) {
                    ...
                    point[`${prefix}_principal`] =
                        monthData.cumulativePrincipal;
                    point[`${prefix}_total`] = monthData.cumulativeTotal;
                    point[`${prefix}_payment`] = monthData.totalPayment;
                    point[`${prefix}_interestPortion`] =
                        monthData.interestPortion;
                    point[`${prefix}_principalPortion`] =
                        monthData.principalPortion;
