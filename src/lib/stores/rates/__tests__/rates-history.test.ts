...
describe("reconstructRatesAtDate", () => {
	...
	it("applies remove operation from changeset", () => {
		const history = createHistoryFile(
			{
				timestamp: "2024-06-01T00:00:00.000Z",
				rates: [createRate({ id: "rate-1" }), createRate({ id: "rate-2" })],
			},
			[
				{
					timestamp: "2024-07-01T00:00:00.000Z",
	...
	it("applies update operation from changeset", () => {
		const history = createHistoryFile(
			{
				timestamp: "2024-06-01T00:00:00.000Z",
				rates: [createRate({ id: "rate-1", rate: 3.5, name: "Original" })],
			},
			[
				{
					timestamp: "2024-07-01T00:00:00.000Z",
					...
					operations: [
						{
							op: "update",
							id: "rate-1",
							changes: { id: "rate-1", rate: 3.25, name: "Updated" },
						},
					],
				},
			],
			...
			[
				{
					...
					afterHash: "hash-1",
					operations: [
						{ op: "update", id: "rate-1", changes: { id: "rate-1", apr: 3.7 } },
					],
				},
			],
		);
...
describe("getRateChanges", () => {
	...
	it("tracks field changes in update", () => {
		const history = createHistoryFile(
			{
				timestamp: "2024-01-01T00:00:00.000Z",
				rates: [createRate({ id: "rate-1", rate: 3.5, minLtv: 0, maxLtv: 80 })],
			},
			[
				{
					timestamp: "2024-06-15T00:00:00.000Z",
		...
		const rateChange = result[0].fieldChanges?.find((f) => f.field === "rate");
		expect(rateChange?.previousValue).toBe(3.5);
		expect(rateChange?.newValue).toBe(3.25);

		const ltvChange = result[0].fieldChanges?.find((f) => f.field === "maxLtv");
