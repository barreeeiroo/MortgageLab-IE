...
describe("getIncorrectRateUrl", () => {
	...
	it("includes additional context when provided", () => {
		...
		expect(urlContains(url, "Rate shown is 3.5% but website shows 3.25%")).toBe(
			true,
		);
	});
...
describe("getNewRateUrl", () => {
	it("generates correct URL with minimal parameters", () => {
		...
		expect(urlContains(url, "_Reported from Rate Updates dialog_")).toBe(true);
	});
	...
	it("includes rate description when provided", () => {
		...
		expect(urlContains(url, "rate-description=5 Year Fixed Green Rate")).toBe(
			true,
		);
	});
...
describe("getBugReportUrl", () => {
	...
	it("includes expected behaviour when provided", () => {
		...
		expect(urlContains(url, "expected-behaviour=Should open modal")).toBe(true);
	});
...
describe("getFeatureRequestUrl", () => {
	...
	it("includes description when provided", () => {
		...
		expect(urlContains(url, "description=Add dark mode support")).toBe(true);
	});
