...
describe("calculateCashbackAmount", () => {
	it("calculates flat cashback correctly", () => {
		expect(calculateCashbackAmount(300000, { type: "flat", value: 5000 })).toBe(
			5000,
		);
		expect(calculateCashbackAmount(500000, { type: "flat", value: 3000 })).toBe(
			3000,
		);
	});
	...
	it("applies cap to flat cashback", () => {
		expect(
			calculateCashbackAmount(300000, { type: "flat", value: 5000, cap: 3000 }),
		).toBe(3000);
	});
});
