...
describe("isValidFollowOnRate", () => {
    ...
    describe("exact LTV matching vs overlap check", () => {
        ...
        it("overlap check rejects adjacent bands (70-80 fixed vs 0-70 variable)", () => {
            ...
            expect(isValidFollowOnRate(fixedRate7080, variableRate070)).toBe(
                false,
            );
        });
        ...
        it("overlap check accepts overlapping bands (70-80 fixed vs 70-90 variable)", () => {
            ...
            expect(isValidFollowOnRate(fixedRate7080, variableRate7090)).toBe(
                true,
            );
        });
        ...
        it("exact LTV matching accepts when LTV is within variable rate range", () => {
            ...
            expect(
                isValidFollowOnRate(fixedRate7080, variableRate070, 69),
            ).toBe(true);
        });
        ...
        it("exact LTV matching rejects when LTV is outside variable rate range", () => {
            ...
            expect(
                isValidFollowOnRate(fixedRate7080, variableRate070, 75),
            ).toBe(false);
        });
        ...
        it("exact LTV at boundary is accepted (LTV equals maxLtv)", () => {
            ...
            expect(
                isValidFollowOnRate(fixedRate7080, variableRate070, 70),
            ).toBe(true);
        });
        ...
        it("exact LTV at boundary is accepted (LTV equals minLtv)", () => {
            ...
            expect(
                isValidFollowOnRate(fixedRate7080, variableRate7090, 70),
            ).toBe(true);
        });
    });
});
...
describe("findVariableRate", () => {
    ...
    describe("follow-on LTV determines variable rate match", () => {
        ...
        it("falls back to flex rate when no follow-on rate matches", () => {
            ...
            const flexOnly = avantVariableRates.filter((r) =>
                r.id.includes("flex"),
            );
...
describe("isRateEligibleForBalance", () => {
    ...
    describe("LTV eligibility", () => {
        ...
        it("returns true at exact boundary values", () => {
            // Exactly 60% LTV
            expect(isRateEligibleForBalance(rate, 24000000, 40000000)).toBe(
                true,
            );
            // Exactly 80% LTV
            expect(isRateEligibleForBalance(rate, 32000000, 40000000)).toBe(
                true,
            );
        });
    });
    ...
    describe("minLoan eligibility (HVM)", () => {
        ...
        it("returns true when balance is above minLoan", () => {
            ...
            const result = isRateEligibleForBalance(
                hvmRate,
                30000000,
                40000000,
            );
