...
export async function exportRentVsBuyToPDF(
    ...
    y = addStyledSectionHeader(doc, "Upfront Costs to Buy", y, {
        divider: true,
    });
    y = addKeyValue(
        doc,
    ...
    if (result.yearlyBreakdown.length > 0) {
        y = addStyledSectionHeader(doc, "Yearly Comparison", y, {
            divider: true,
        });
...
export async function exportRemortgageToPDF(
    ...
    if (filteredYearlyBreakdown.length > 0) {
        y = addStyledSectionHeader(doc, "Yearly Breakdown", y, {
            divider: true,
        });
    ...
    y = addMetricRow(
        ...
        [
            ...
            },
            {
                ...
                value: formatCurrencyForExport(
                    cheapestOption.adjustedBalance,
                    true,
                ),
                variant: "default",
            },
...
function prepareCashbackKeyMetricsTableData(
    ...
    const rows = [
        ...
        [
            "Balance After",
            ...options.map((opt) =>
                formatCurrencyForExport(opt.balanceAtEnd, true),
            ),
        ],
    ];
