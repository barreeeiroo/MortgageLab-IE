...
function prepareSummaryData(context: SimulateExportContext): TableExportData {
    ...
    const rows: (string | number)[][] = [
        [
            "Mortgage Amount",
            formatCurrencyForExport(mortgageAmount / 100, true),
        ],
...
export async function exportSimulationToPDF(
    ...
    if (context.overpaymentConfigs && context.overpaymentConfigs.length > 0) {
        ...
        if (overpaymentsData) {
            y = addStyledSectionHeader(doc, "Overpayments", y, {
                divider: true,
            });
    ...
    if (context.selfBuildConfig?.enabled) {
        const selfBuildData = prepareSelfBuildDataForPDF(
            context.selfBuildConfig,
        );
        if (selfBuildData) {
            ...
            y = addKeyValue(
                ...
                context.selfBuildConfig.constructionRepaymentType ===
                    "interest_only"
                    ? "Interest Only"
                    : "Interest & Capital",
                y,
    ...
    if (context.chartImages && context.chartImages.length > 0) {
        ...
        for (const chart of context.chartImages) {
            ...
            y = addImage(doc, chart.imageDataUrl, y, {
                width: 180,
                height: 90,
            });
        }
