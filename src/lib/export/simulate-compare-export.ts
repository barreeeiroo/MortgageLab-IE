    ...
    const rows = simulations.map((sim) => {
        const ltv =
            sim.input.propertyValue > 0
                ? (
                      (sim.input.mortgageAmount / sim.input.propertyValue) *
                      100
                  ).toFixed(0)
                : "N/A";
            ...
            if (yearData) {
                row.push(
                    formatCurrencyForExport(
                        yearData.closingBalance / 100,
                        true,
                    ),
                );
                row.push(
                    formatCurrencyForExport(yearData.totalInterest / 100, true),
                );
            } else {
                row.push("-");
                row.push("-");
