...
export async function exportAffordabilityToPDF(
    ...
    const vatAddedToTotal =
        vatResult.vatRate > 0 && !(priceIncludesVAT ?? true);
    ...
    if (calculatorType === "mover" && context.currentPropertyValue) {
        y = addStyledSectionHeader(doc, "Current Property", y, {
            divider: true,
        });
        y = addKeyValue(
            doc,
    ...
    if (calculatorType === "btl" && context.rentalIncome) {
        y = addStyledSectionHeader(doc, "Rental Analysis", y, {
            divider: true,
        });
        y = addKeyValue(
            doc,
