...
export async function addTableWithLogos(
    ...
    const tableWidth =
        pageWidth - PDF_LAYOUT.marginLeft - PDF_LAYOUT.marginRight;
    ...
    const tableOptions: any = {
        ...
        didDrawCell: (hookData: {
            ...
            if (
                ...
                hookData.column.index === logoColumn
            ) {
                ...
                if (lenderId) {
                    ...
                    if (logoDataUrl) {
                        try {
                            ...
                            const y =
                                hookData.cell.y +
                                (hookData.cell.height - logoSize) / 2;
                            doc.addImage(
                                logoDataUrl,
                                "PNG",
                                x,
...
export function addFooter(
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.textWithLink(
            linkText,
            PDF_LAYOUT.marginLeft + prefixWidth,
            footerY,
            {
        ...
        doc.text(
            suffix,
            PDF_LAYOUT.marginLeft + prefixWidth + linkWidth,
            footerY,
        );

        // Page number (right)
        doc.text(`Page ${i} of ${pageCount}`, rightX, footerY, {
            align: "right",
        });
    }
}
