...
describe("CustomPerkForm", () => {
	...
	describe("edit mode", () => {
		...
		it("pre-populates form with existing perk data", () => {
			render(<CustomPerkForm {...defaultProps} initialPerk={existingPerk} />);

			expect(screen.getByDisplayValue("Existing Perk")).toBeInTheDocument();
			expect(screen.getByDisplayValue("An existing perk")).toBeInTheDocument();
		});
