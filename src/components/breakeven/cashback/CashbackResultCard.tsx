...
export function CashbackResultCard({
    ...
    const worstOption = result.options.reduce(
        (worst, opt) =>
            opt.adjustedBalance > worst.adjustedBalance ? opt : worst,
        result.options[0],
    );
        ...
        <div className="space-y-4">
            ...
            <div className="p-3 rounded-lg bg-muted/50 border">
                ...
                {!result.allVariable && (
                    <p className="text-xs text-muted-foreground mt-1">
                        Fixed rates only apply during their fixed period, so we
                        compare up to the longest fixed period.
                    </p>
                )}
            </div>
            ...
            <Card className="bg-primary/5 border-primary/20">
                <CardContent>
                        ...
                        <div className="p-3 rounded-lg bg-background border">
                            <div className="flex items-center justify-between">
                                <span className="text-sm text-muted-foreground">
                                    Saves over comparison period vs{" "}
                                    {worstOption.label}
                                </span>
                            ...
                            <thead>
                                <tr className="border-b">
                                    ...
                                    {result.options.map((opt, index) => {
                                        const isWinner =
                                            index ===
                                            result.cheapestAdjustedBalanceIndex;
                                        return (
                                            <th
                                                key={opt.label}
                    ...
                    {/* Breakeven Points */}
                    {result.breakevens.some(
                        (be) => be.breakevenMonth !== null,
                    ) && (
                        <div>
                    ...
                    {overpaymentAllowances &&
                        overpaymentAllowances.length > 0 && (
                            <div className="mt-6">
                                <p className="text-sm font-medium mb-3">
                                    Overpayment Allowances (
                                    {result.comparisonPeriodYears}y)
                                </p>
                                ...
                                <p className="text-xs text-muted-foreground mt-2">
                                    Overpayment allowance per lender policy.
                                    Exceeding may incur charges.
                                </p>
                            </div>
                        )}
                </CardContent>
...
function MetricCaption({
    ...
    switch (metric) {
        ...
        case "adjustedBalances":
            return (
                <>
                    Adjusted balance = balance if cashback applied to principal
                    at start.
                    {projectionNote}
                </>
            );
    ...
    switch (metric) {
            ...
            return (
                <>
                    ...
                    <p>
                        ...
                        will have {formatCurrency(winner.balanceAtEnd)}{" "}
                        remaining
                    </p>
                    <p>
                        •{" "}
                        ...
                        will have {formatCurrency(runnerUp.balanceAtEnd)}{" "}
                        remaining
                    </p>
                    <p className="pt-1">
                        The {formatCurrency(diff)} difference comes from
                        different principal payments due to different rates. A{" "}
                        {winner.rate < runnerUp.rate ? "lower" : "higher"} rate
                        means {winner.rate < runnerUp.rate ? "more" : "less"}{" "}
        ...
        case "netCosts":
            return (
                <>
                    <p>
                        <strong>Net cost</strong> = Interest paid − Cashback
                        received
                    </p>
                    <p className="pt-1">
                        •{" "}
        ...
        case "adjustedBalances":
            return (
                <>
                    <p>
                        <strong>Adjusted balance</strong> = Balance if cashback
                        was applied to principal at start
                    </p>
                    <p className="text-muted-foreground">
                        (Shows your effective debt if you use cashback to reduce
                        your mortgage immediately)
                    </p>
                    <p className="pt-1">
                        •{" "}
                        <strong className="text-foreground">
