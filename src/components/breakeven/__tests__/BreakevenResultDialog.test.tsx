...
describe("BreakevenResultDialog", () => {
	...
	describe("action buttons", () => {
		...
		it("hides Export and Share buttons when no result", () => {
			render(<BreakevenResultDialog {...defaultProps} hasResult={false} />);
	...
	describe("export button", () => {
		it("calls onExport when clicked", async () => {
			...
			render(<BreakevenResultDialog {...defaultProps} onExport={onExport} />);
		...
		it("shows Exporting... when isExporting is true", () => {
			render(<BreakevenResultDialog {...defaultProps} isExporting={true} />);
	...
	describe("dialog closed state", () => {
		it("does not render when closed", () => {
			...
			expect(
				screen.queryByRole("heading", { name: /rent vs buy analysis/i }),
			).not.toBeInTheDocument();
		});
	});
});
