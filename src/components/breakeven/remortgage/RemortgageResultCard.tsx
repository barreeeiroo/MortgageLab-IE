                        ...
                        <div className="p-3 rounded-lg bg-background border">
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                        When monthly savings exceed switching
                                        costs
                                    </p>
                                </div>
                                <p
                            ...
                            {result.breakevenDetails && hasBreakeven && (
                                <details className="mt-2 group">
                                    ...
                                    <div className="mt-2 text-xs text-muted-foreground space-y-1 pl-4 border-l-2 border-muted">
                                        ...
                                        <p>
                                            After {breakevenText}, you'll have
                                            saved{" "}
                                            <strong className="text-green-600">
                                                {formatCurrency(
                                                    result.breakevenDetails
                                                        .cumulativeSavingsAtBreakeven,
                                                )}
                                            </strong>
                                    ...
                                    {filteredYearlyBreakdown.length > 0 && (
                                        <SavingsBreakevenChart
                                            ...
                                            monthlyData={result.monthlyBreakdown.filter(
                                                (m) =>
                                                    m.month <=
                                                    result.comparisonPeriodMonths,
                                            )}
                                            switchingCosts={
                                                result.switchingCosts
                                            }
                            ...
                            {!hasBreakeven && result.monthlySavings <= 0 && (
                                <p className="mt-2 text-xs text-amber-600">
                                    The new rate doesn't offer monthly savings
                            ...
                            {breakevenExceedsCurrentFixed && (
                                <div className="mt-2 p-2 rounded bg-amber-500/10 border border-amber-500/30">
                                    <p className="text-xs text-amber-700 dark:text-amber-400">
                                        <strong>Consider waiting:</strong>{" "}
                                        Breakeven ({breakevenText}) exceeds your
                                        current rate's remaining{" "}
                                        {formatBreakevenPeriod(
                                            currentRateRemainingFixedMonths,
                                        )}{" "}
                                        fixed period. You could wait until it
                                        ends to avoid any early repayment
                                        charges.
                                    </p>
                                </div>
                            )}
                            {breakevenExceedsNewFixedPeriod && (
                                <div className="mt-2 p-2 rounded bg-amber-500/10 border border-amber-500/30">
                                    <p className="text-xs text-amber-700 dark:text-amber-400">
                                        <strong>Warning:</strong> Breakeven (
                                        {breakevenText}) exceeds the new rate's{" "}
                                        {newRateFixedPeriodYears}-year fixed
                                        period. After the fixed period ends,
                                        your rate may change, affecting your
                                        actual savings.
                                    </p>
                                </div>
                            )}
                            {isNewRateVariable && hasBreakeven && (
                                <div className="mt-2 p-2 rounded bg-blue-500/10 border border-blue-500/30">
                                    <p className="text-xs text-blue-700 dark:text-blue-400">
                                        <strong>Note:</strong> Variable rates
                                        can change at any time based on market
                                        conditions. Your actual breakeven may be
                                        shorter or longer depending on future
                                        rate changes by the lender.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-3 rounded-lg bg-background border">
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                        Interest you'll avoid paying over the
                                        term
                                    </p>
                                </div>
                                <p
                ...
                {(year1 || year5 || year10) && (
                    <div className="pt-4 border-t border-border mt-4">
                        ...
                        <p className="text-xs text-muted-foreground mb-3">
                            Net savings = cumulative monthly savings − switching
                            costs.
                        </p>
                        <div className="grid gap-4 sm:grid-cols-3 text-sm">
                            {year1 && (
                                <RemortgageYearSnapshot year={1} data={year1} />
                            )}
                            {year5 && (
                                <RemortgageYearSnapshot year={5} data={year5} />
                            )}
                            {year10 && (
                                <RemortgageYearSnapshot
                                    year={10}
                                    data={year10}
                                />
                            )}
        ...
        <div className="p-3 rounded-lg bg-background border">
            ...
            <div className="space-y-1 text-xs">
                ...
                <div className="flex justify-between">
                    ...
                    <span>
                        {formatCurrency(
                            data.remainingBalanceCurrent -
                                data.remainingBalanceNew,
                        )}
                    </span>
                </div>
            </div>
