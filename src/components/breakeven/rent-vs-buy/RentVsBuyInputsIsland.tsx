...
import { showRentVsBuyResult } from "@/lib/stores/breakeven";
import {
    formatCurrency,
    formatCurrencyInput,
    parseCurrency,
    ...
    useEffect(() => {
        ...
        if (hasBreakevenShareParam()) {
            ...
            if (shared && shared.type === "rvb") {
                ...
                if (shared.rentInflation)
                    setRentInflation(shared.rentInflation);
                if (shared.homeAppreciation)
                    setHomeAppreciation(shared.homeAppreciation);
                if (shared.maintenanceRate)
                    setMaintenanceRate(shared.maintenanceRate);
                if (shared.opportunityCost)
                    setOpportunityCost(shared.opportunityCost);
                if (shared.saleCost) setSaleCost(shared.saleCost);
                if (shared.serviceCharge)
                    setServiceCharge(shared.serviceCharge);
        ...
        <Card>
            <CardContent>
                <div className="mb-6">
                    ...
                    <CardDescription>
                        Compare the long-term costs of renting versus buying a
                        home. Factor in rent increases, home appreciation, and
                        building equity.
                    </CardDescription>
                </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {hasPropertyValue && (
                                <p className="text-xs text-muted-foreground">
                                    Stamp Duty:{" "}
                                    {formatCurrency(
                                        calculateStampDuty(vatResult.netPrice),
                                    )}
                                    {hasVAT &&
                                        (priceIncludesVAT
                        ...
                        >
                            ...
                            >
                                ...
                                <p className="text-xs text-muted-foreground">
                                    ...
                                    only. When your fixed rate period ends,
                                    you'll need to remortgage to secure a new
                                    fixed rate and avoid reverting to a
                                    potentially higher variable rate. See our{" "}
                                    <a
                            ...
                            <div className="space-y-4 mt-2">
                                ...
                                <div className="p-4 border rounded-lg bg-muted/30">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Service charges are typically for
                                            apartments/managed developments.
                                        </p>
                                    </div>
                                </div>
                            </div>
