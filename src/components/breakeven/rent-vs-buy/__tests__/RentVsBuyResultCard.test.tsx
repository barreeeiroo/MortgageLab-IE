...
describe("RentVsBuyResultCard", () => {
	...
	describe("breakeven display", () => {
		it("displays Net Worth Breakeven section", () => {
			...
			expect(
				screen.getByText(/when buying is financially better than renting/i),
			).toBeInTheDocument();
		});
		...
		it("displays Break-even on Sale section", () => {
			...
			expect(
				screen.getByText(/when you can sell and recover your upfront costs/i),
			).toBeInTheDocument();
			// 48 months = 4 years
		...
		it("displays 'Never' when no breakeven is reached", () => {
			...
			render(<RentVsBuyResultCard {...defaultProps} result={noBreakeven} />);
	...
	describe("monthly comparison section", () => {
		it("displays current monthly rent", () => {
			...
			expect(screen.getByText("Current Monthly Rent")).toBeInTheDocument();
	...
	describe("card styling", () => {
		it("applies primary styling when breakeven is reached", () => {
			const { container } = render(<RentVsBuyResultCard {...defaultProps} />);
