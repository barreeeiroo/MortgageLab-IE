							...
							<div className="border-border/50 bg-background rounded-lg border px-3 py-2 shadow-xl min-w-[200px]">
								...
								<div className="space-y-2">
									{simulations.map((sim) => {
										...
										const recurringOp =
											dataPoint[`${sim.id}_recurringOverpayment`];
										const oneTimeOp = dataPoint[`${sim.id}_oneTimeOverpayment`];
											...
											<div key={sim.id} className="space-y-1">
												...
												<div className="pl-4 space-y-0.5">
													...
													{showRecurringOverpayment &&
														recurringOp !== undefined &&
														(recurringOp as number) > 0 && (
															<div className="flex justify-between text-sm">
																<span className="text-emerald-600 dark:text-emerald-400">
																	Recurring Overpay
																</span>
																<span className="font-mono">
																	{formatChartCurrency(recurringOp as number)}
																</span>
															</div>
														)}
													{showOneTimeOverpayment &&
														oneTimeOp !== undefined &&
														(oneTimeOp as number) > 0 && (
															<div className="flex justify-between text-sm">
																<span className="text-teal-600 dark:text-teal-400">
																	One-time Overpay
																</span>
																<span className="font-mono">
																	{formatChartCurrency(oneTimeOp as number)}
																</span>
															</div>
														)}
				...
				{showInterest &&
					simulations.map((sim) => (
						<Bar
							...
							stackId={sim.id}
							radius={hasSegmentsAboveInterest ? [0, 0, 0, 0] : [4, 4, 0, 0]}
							isAnimationActive={animate}
							animationDuration={ANIMATION_DURATION}
							barSize={barWidth}
						/>
