...
export function SimulateTable({
    ...
    for (const milestone of milestones) {
        const year =
            findYearForMonth(milestone.month) ??
            Math.ceil(milestone.month / 12);
    ...
    for (const config of overpaymentConfigs) {
        ...
        for (const yearData of yearlySchedule) {
            ...
            const yearEndMonth =
                yearData.months[yearData.months.length - 1]?.month ??
                yearStartMonth;

            // Check if overpayment is active during this year
            if (
                config.startMonth <= yearEndMonth &&
                ...
                yearCounts.set(
                    displayLabel,
                    (yearCounts.get(displayLabel) || 0) + 1,
                );
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="flex items-center gap-1 text-red-600 dark:text-red-500 cursor-help">
                                            <span className="font-medium">
                                                {formatEuro(
                                                    summary.extraInterestFromSelfBuild,
                                                )}{" "}
                                                extra
                                            </span>
                                    ...
                                    <TooltipContent>
                                        <p>
                                            Extra interest vs paying principal
                                            during construction
                                        </p>
                                    </TooltipContent>
                                </Tooltip>
                        ...
                        {summary.monthsSaved > 0 && (
                            <Tooltip>
                                ...
                                <TooltipContent>
                                    <p>
                                        {formatEuro(summary.interestSaved)} in
                                        interest saved with your overpayments
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                        )}
                        ...
                        <TableBody>
                            {yearlySchedule.map((year) => (
                                <SimulateYearRow
                                    ...
                                    warnings={
                                        warningsByYear.get(year.year) || []
                                    }
                                    milestones={
                                        milestonesByYear.get(year.year) || []
                                    }
                                    ratePeriodLabels={ratePeriodLabels}
                                    overpaymentLabels={
                                        overpaymentLabelsByYear.get(
                                            year.year,
                                        ) || []
                                    }
