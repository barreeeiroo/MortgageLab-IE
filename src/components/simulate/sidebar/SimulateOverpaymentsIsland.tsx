...
export function SimulateOverpaymentsIsland() {
    ...
    >(null);
    const [deletingOverpayment, setDeletingOverpayment] = useState<
        string | null
    >(null);
    ...
    const groupedOverpayments = useMemo(() => {
        ...
        for (const [periodId, overpayments] of groups) {
            ...
            sortedGroups.push({
                periodId,
                period,
                overpayments: overpayments.sort(
    ...
    const formatGroupHeader = (
        ...
        const dateStr = formatTransitionDate(
            input.startDate,
            period.startMonth,
            {
                short: true,
        ...
        <Card className="py-0 gap-0">
            ...
            <CardContent className="pt-0 px-3 pb-3">
                {groupedOverpayments.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {groupedOverpayments.map(
                            ({ periodId, period, overpayments }) => (
                                    ...
                                    {overpayments.map((config) => {
                                        ...
                                        return (
                                            <SimulateOverpaymentEvent
                                                ...
                                                onEdit={() =>
                                                    setEditingOverpayment(
                                                        config,
                                                    )
                                                }
                                                onDelete={() =>
                                                    setDeletingOverpayment(
                                                        config.id,
                                                    )
                                                }
                                                onToggleEnabled={() =>
            ...
            {editingOverpayment && (
                <SimulateEditOverpaymentDialog
                    ...
                    onOpenChange={(open) =>
                        !open && setEditingOverpayment(null)
                    }
                    onSave={(config) => {
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will remove this overpayment configuration from
                            your simulation. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={showDeleteAll} onOpenChange={setShowDeleteAll}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will remove all {overpaymentConfigs.length}{" "}
                            overpayment configurations from your simulation.
