...
export function SimulateAddOverpaymentDialog({
    ...
    const availableFixedPeriods = useMemo(
        () =>
            fixedRatePeriods.filter(
                (p) => !periodsWithMaxOverpayment.has(p.id),
            ),
    ...
    const customRatePeriodBounds = useMemo(() => {
        const period = resolvedRatePeriods.find(
            (p) => p.id === customRatePeriodId,
        );
    ...
    const isCustomPeriodFixed =
        customRatePeriodBounds?.period?.type === "fixed";
            ...
            if (firstPeriod) {
                const periodEndMonth =
                    firstPeriod.durationMonths === 0
                        ? totalMonths
                        : firstPeriod.startMonth +
                          firstPeriod.durationMonths -
                          1;
    ...
    const handleMaximizeSubmit = () => {
        if (yearlyPlans.length === 0 || !selectedPeriod || !selectedPolicy)
            return;

        // For constant-allowance policies (single plan), use simpler label
        const isSinglePlan =
            yearlyPlans.length === 1 &&
            isConstantAllowancePolicy(selectedPolicy);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-lg max-h-[85vh] flex flex-col">
                <DialogHeader className="shrink-0">
                    ...
                    <DialogDescription>
                        Configure an overpayment to see how it affects your
                        mortgage.
                    </DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter className="shrink-0 border-t pt-4">
                    ...
                    {mode === "maximize" ? (
                        <Button
                            onClick={handleMaximizeSubmit}
                            disabled={!isMaximizeValid}
                        >
                            Add Overpayment
                        </Button>
                    ) : (
            ...
            {isDelayedByConstruction && (
                <div className="flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm dark:border-blue-900 dark:bg-blue-950">
                    ...
                    <p className="text-blue-800 dark:text-blue-200">
                        ...
                        {constructionEndMonth}) to avoid issues with overpayment
                        allowance calculations while the mortgage isn't fully
                        drawn down.
                    </p>
                </div>
            )}
            ...
            {availableFixedPeriods.length === 0 ? (
                <div className="rounded-lg border border-muted bg-muted/50 p-4">
                    <p className="text-sm text-muted-foreground">
                        ...
                        configured. Use the One-time or Recurring tabs to add
            ...
            {selectedPeriod && !selectedPolicy && (
                <div className="rounded-lg border border-yellow-500/50 bg-yellow-50 dark:bg-yellow-950/20 p-4">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                        This lender doesn't have an overpayment policy
                        configured. Use the One-time or Recurring tabs to
                        manually enter an overpayment.
                    </p>
                </div>
            )}
        </>
