...
export function RateTimelineChart({
	...
	const minLtv = Math.max(0, Math.floor(Math.min(...ltvValues) / 10) * 10 - 10);
		...
		.map((m) => {
			...
			if (granularity === "monthly") {
				...
				if (startDate) {
					...
					const targetYear =
						startYear + Math.floor((startMonth - 1 + m.month - 1) / 12);

					// Find the data point with this calendar year
					periodData = data.find(
						(d) => d.calendarYear === targetYear || d.period === targetYear,
					);
				...
				>
										...
										{dataPoint.rate !== undefined && (
											<div className="flex items-center justify-between gap-4">
												...
												<span className="font-mono font-medium text-sm">
													{formatPercentage(dataPoint.rate)}
												</span>
											</div>
										)}
										{/* LTV */}
										{dataPoint.ltv !== undefined && (
											<div className="flex items-center justify-between gap-4">
												<div className="flex items-center gap-1.5">
													<div
														className="h-2.5 w-2.5 rounded-sm shrink-0"
														style={{ backgroundColor: CHART_COLORS.ltv }}
													/>
													<span className="text-muted-foreground text-sm">
														LTV
													</span>
												...
												<span className="font-mono font-medium text-sm">
													{formatPercentage(dataPoint.ltv)}
												</span>
											</div>
										)}
					...
					{mappedRatePeriods.map((period, idx) => {
						...
						const x2 = nextPeriod
							...
							: (data[data.length - 1]?.period ?? period.chartEndPeriod);
					...
					{mappedRatePeriods.map((period) => (
						<div key={period.id} className="flex items-center gap-1.5">
							<div
								...
								style={{
									backgroundColor: period.color.replace("0.08", "0.3"),
								}}
							/>
