				...
				<ChartTooltip
					content={({ active, payload }) => {
						...
						const balanceSavings =
							(dataPoint.baselineBalance ?? 0) - dataPoint.principalRemaining;
						...
						return (
							<div className="border-border/50 bg-background rounded-lg border px-3 py-2 shadow-xl">
								...
								<div className="space-y-1">
									...
									{visibility.baseline &&
										dataPoint.baselineBalance !== undefined && (
											<div className="flex items-center justify-between gap-4">
												<div className="flex items-center gap-1.5">
													<div
														className="h-2.5 w-2.5 rounded-sm shrink-0"
														style={{ backgroundColor: CHART_COLORS.baseline }}
													/>
												...
												<span className="font-mono font-medium text-sm">
													{formatChartCurrency(dataPoint.baselineBalance)}
												</span>
											</div>
										)}
									{visibility.actual && (
										<div className="flex items-center justify-between gap-4">
											...
											<span className="font-mono font-medium text-sm">
												{formatChartCurrency(dataPoint.principalRemaining)}
											</span>
										</div>
									)}
									...
									{visibility.interestBaseline &&
										dataPoint.baselineCumulativeInterest !== undefined && (
											<div className="flex items-center justify-between gap-4">
												<div className="flex items-center gap-1.5">
													<div
														className="h-2.5 w-2.5 rounded-sm shrink-0"
										...
										<div className="flex items-center justify-between gap-4">
											<div className="flex items-center gap-1.5">
												<div
													className="h-2.5 w-2.5 rounded-sm shrink-0"
													style={{
														backgroundColor: CHART_COLORS.interestSaved,
													}}
												/>
												<span className="text-muted-foreground text-sm">
													Actual
											...
											<span className="font-mono font-medium text-sm">
												{formatChartCurrency(dataPoint.cumulativeInterest)}
											</span>
										</div>
									)}

									{/* Savings summary */}
									{(balanceSavings > 0 || interestSavings > 0) && (
											...
											{balanceSavings > 0 && (
												<div className="flex items-center justify-between gap-4">
													...
													<span className="font-mono font-medium text-sm text-green-600">
														{formatChartCurrency(balanceSavings)}
													</span>
												</div>
											)}
											{interestSavings > 0 && (
												<div className="flex items-center justify-between gap-4">
													...
													<span className="font-mono font-medium text-sm text-green-600">
														{formatChartCurrency(interestSavings)}
													</span>
												</div>
											)}
										</div>
