...
export function SimulateChartIsland() {
    ...
    const chartData: ChartDataPoint[] = (() => {
        if (granularity === "yearly") {
            return yearlySchedule.map((row) => {
                ...
                for (const m of row.months) {
                    const rp = resolvedRatePeriods.find(
                        (p) => p.id === m.ratePeriodId,
                    );
                    if (rp) {
                        totalRate += rp.rate;
                        rateCount++;
                ...
                const avgRate =
                    rateCount > 0 ? totalRate / rateCount : undefined;
                ...
                const hasInterestOnlyMonth = row.months.some(
                    (m) => m.isInterestOnly,
                );
                ...
                return {
                    ...
                    ratesInPeriod:
                        ratesInPeriod.length > 1 ? ratesInPeriod : undefined,
                    ltv:
                    ...
                    // Self-build fields
                    drawdownThisMonth:
                        yearDrawdowns > 0 ? yearDrawdowns : undefined,
                    cumulativeDrawn:
        ...
        if (granularity === "quarterly") {
            ...
            for (let i = 0; i < monthlySchedule.length; i += 3) {
                ...
                const lastDate = lastMonth.date
                    ? new Date(lastMonth.date)
                    : undefined;

                const calendarMonth = lastDate
                    ? lastDate.getMonth() + 1
                    : undefined;
                ...
                quarters.push({
                    ...
                    ltv:
                        input.propertyValue > 0
                            ? (lastMonth.closingBalance / input.propertyValue) *
                              100
                            : undefined,
                    // Self-build fields
                    drawdownThisMonth:
                        qtrDrawdowns > 0 ? qtrDrawdowns : undefined,
                    cumulativeDrawn:
    ...
    const overpaymentImpactData: ChartDataPoint[] = (() => {
        ...
        if (granularity === "yearly") {
            ...
            for (
                let year = actualLastYear + 1;
                year <= baselineLastYear;
                year++
            ) {
                ...
                extensionData.push({
                    ...
                    baselineCumulativeInterest:
                        lastMonthData.cumulativeInterest,
                });
            }
            ...
            for (
                let qtr = actualLastQuarter + 1;
                qtr <= baselineLastQuarter;
                qtr++
            ) {
