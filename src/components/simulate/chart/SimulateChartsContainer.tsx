        ...
        const captureAllCharts = async () => {
            ...
            try {
                for (const chartType of chartTypes) {
                    ...
                    if (element) {
                        try {
                            const imageDataUrl = await elementToJpegDataUrl(
                                element,
                                {
        ...
        <>
                        ...
                        {activeChart === "cumulative_costs" && (
                            <div className="flex items-center gap-1.5 ml-auto">
                                <Checkbox
                                    id="stacked-checkbox"
                                    checked={
                                        visibility.cumulative_costs.stacked
                                    }
                                    onCheckedChange={() =>
                                        onToggleVisibility(
                                            "cumulative_costs",
                        ...
                        {activeChart === "rate_timeline" && (
                            ...
                            >
                                <div
                                    className="h-2.5 w-2.5 rounded-sm shrink-0"
                                    style={{
                                        backgroundColor: CHART_COLORS.rate,
                                    }}
                                />
                                Rate
                            </Toggle>
            ...
            {pendingCapture && (
                ...
                >
                    ...
                    >
                        <StaticChartLegend
                            items={VISIBILITY_CONFIGS.balance_equity}
                        />
                        <BalanceEquityChart
                            data={data}
                            visibility={visibility.balance_equity}
                    ...
                    >
                        <StaticChartLegend
                            items={VISIBILITY_CONFIGS.payment_breakdown}
                        />
                        <PaymentBreakdownChart
                            data={data}
                            visibility={visibility.payment_breakdown}
                    ...
                    >
                        <StaticChartLegend
                            items={VISIBILITY_CONFIGS.cumulative_costs}
                        />
                        <CumulativeCostsChart
                            data={data}
                            visibility={visibility.cumulative_costs}
                    ...
                    >
                        <StaticChartLegend
                            items={VISIBILITY_CONFIGS.overpayment_impact}
                        />
                        <OverpaymentImpactChart
                            data={overpaymentImpactData}
                            visibility={visibility.overpayment_impact}
                    ...
                    >
                        <StaticChartLegend
                            items={VISIBILITY_CONFIGS.rate_timeline}
                        />
                        <RateTimelineChart
                            data={data}
                            visibility={visibility.rate_timeline}
