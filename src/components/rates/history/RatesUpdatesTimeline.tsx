...
export function RatesUpdatesTimeline({
    ...
    const [expandedFieldChanges, setExpandedFieldChanges] = useState<
        Set<string>
    >(new Set());
    ...
    const allChanges = useMemo(() => {
        ...
        for (const [lenderId, history] of historyData) {
            ...
            if (
                ...
                !filter.lenderIds.includes(lenderId)
            ) {
                continue;
            }
                        ...
                        {(() => {
                            ...
                            return groupedChanges.map((group) => {
                                ...
                                const isExpanded =
                                    ...
                                    groupedChanges.length <= 3;
                                const visibleChanges = isExpanded
                                    ? group.changes
                                    : group.changes.slice(0, 3);
                                const hasMore =
                                    group.changes.length > 3 && !isExpanded;
                                    ...
                                    <div
                                        ...
                                        ref={
                                            isFirstOfYear
                                                ? (el) => {
                                                      if (el) {
                                                          yearRefs.current.set(
                                                              groupYear,
                                                              el,
                                                          );
                                                      } else {
                                                          yearRefs.current.delete(
                                                              groupYear,
                                                          );
                                                      }
                                                  }
