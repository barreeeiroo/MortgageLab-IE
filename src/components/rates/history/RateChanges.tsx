...
export function RateChanges({ historyData, lenders, perks }: RateChangesProps) {
    ...
    const endRates = useMemo(() => {
        ...
        for (const [lenderId, history] of historyData) {
            ...
            if (
                selectedLenderIds.length > 0 &&
                !selectedLenderSet.has(lenderId)
            )
                continue;
    ...
    const comparisonRates = useMemo(() => {
        ...
        for (const [lenderId, history] of historyData) {
            ...
            const historicalRates = reconstructRatesAtDate(
                history,
                comparisonDate,
            );
    ...
    const stats = useMemo(() => {
        ...
        const modified = searchFilteredRates.filter(
            (r) => r.status === "modified",
        );
        ...
        const removed = searchFilteredRates.filter(
            (r) => r.status === "removed",
        );
