...
function getComparisonValue(rate: CompareRateRow, key: ComparisonKey): string {
    switch (key) {
        ...
        case "followOnProduct":
            return ""; // Handled specially in render
        case "followOnRate":
            return rate.followOnRate
                ? `${rate.followOnRate.rate.toFixed(2)}%`
                : "—";
...
export function CompareRatesModal({
    ...
    const cashbackComparisonState = useMemo(() => {
        ...
        const configKeys = cashbackConfigs.map((config) =>
            config
                ? `${config.type}|${config.value}|${config.cap ?? ""}`
                : "none",
        );
                        ...
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <DialogDescription>
                                    ...
                                    over {formatTermDisplay(mortgageTerm)}
                                </DialogDescription>
                            </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {COMPARISON_ROWS.map((row) => {
                                const { bestIndices, worstIndices } =
                                    getBestWorstIndices(rates, row.key);
                                ...
                                return (
                                    <TableRow key={row.key}>
                                        ...
                                        {rates.map((rate, index) => {
                                            ...
                                            const hasWarning =
                                                showWarnings &&
                                                rate.usesFixedRateForWholeTerm;
                    ...
                    {cashbackComparisonState && (
                        <div className="flex justify-end mt-4">
                            <Tooltip>
                                ...
                                <TooltipContent>
                                    <p>
                                        Compare cashback offers in the Cashback
                                        Calculator
                                    </p>
                                </TooltipContent>
                            </Tooltip>
