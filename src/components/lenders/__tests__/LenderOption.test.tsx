...
describe("LenderOption", () => {
	...
	describe("custom size", () => {
		it("passes size to LenderLogo", () => {
			...
			expect(logoContainer).toHaveStyle({ width: "32px", height: "32px" });
		});

		it("uses default size of 20", () => {
			const { container } = render(<LenderOption lenderId="aib" name="AIB" />);

			const logoContainer = container.querySelector("[style*='width']");
			expect(logoContainer).toHaveStyle({ width: "20px", height: "20px" });
		});
	});
