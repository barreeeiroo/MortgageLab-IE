...
describe("BuyToLetCalculator", () => {
	...
	describe("form rendering", () => {
		...
		it("renders available deposit input", () => {
			...
			expect(screen.getByLabelText("Available Deposit")).toBeInTheDocument();
		});
		...
		it("does not render self-build checkbox (BTL specific)", () => {
			...
			expect(screen.queryByLabelText(/Self Build/)).not.toBeInTheDocument();
		});
	});
	...
	describe("form validation", () => {
		...
		it("requires deposit to be entered", async () => {
			...
			await user.type(screen.getByLabelText("Expected Monthly Rent"), "1500");
		...
		it("requires expected rent to be entered", async () => {
			...
			await user.type(screen.getByLabelText(/Gross Annual Salary/i), "80000");
			await user.type(screen.getByLabelText("Available Deposit"), "100000");
			await fillDateOfBirth(user);
	...
	describe("calculation and result", () => {
		...
		it("shows rental constrained title when limited by rental income", async () => {
			...
			await user.type(screen.getByLabelText(/Gross Annual Salary/i), "200000");
			await user.type(screen.getByLabelText("Available Deposit"), "200000");
			await user.type(screen.getByLabelText("Expected Monthly Rent"), "500");
			await fillDateOfBirth(user);
			...
			await waitFor(() => {
				expect(
					screen.getByText("Mortgage Summary (Limited by Rental Income)"),
				).toBeInTheDocument();
			});
		});
		...
		it("displays rental analysis section in result", async () => {
			...
			await waitFor(() => {
				...
				expect(screen.getByText("Est. Mortgage Payment")).toBeInTheDocument();
				expect(
	...
	describe("stress test information", () => {
		it("displays stress test rate information in result", async () => {
			...
			await waitFor(() => {
				...
				expect(screen.getByText(/stress-tested rate/)).toBeInTheDocument();
			});
		});
	});
});
